...
dds_bray_dm <- vegdist(t(nit[,-c(1,ncol(nit))]), method="bray", na.rm=T)
...
dds_dist = as.dist(dds_bray_dm)
dds_metaMDS<-metaMDS(dds_dist, distance = "bray", k = 2, trymax = 1000, autotransform =FALSE, noshare = 0.1, wascores = TRUE, expand = TRUE, trace = 1, plot = TRUE)
adonis(dds_dist ~ location * treatment * harvest, data = mtx_design, strata = mtx_design$harvest, permutations = 10000)
...
betas <- res_all_data_bulkcomp_sig[, c(1:14, ncol(res_all_data_bulkcomp_sig))]
...
```
```{r}
# Figure 4 panel C - heatmap of upregulation of GS/GOGAT regulatory genes
reg <- betas %>%
